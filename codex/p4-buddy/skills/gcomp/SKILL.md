---
name: gcomp
description: Generates a <hr-platform> mission report from the active project or a user hint. Infers all fields automatically and confirms each section before finalizing.
---

# <hr-platform> Mission Generator
...
- **Without argument**: analyze the active workspace (git log, CLAUDE.md, README, file structure) and infer all fields.
...
| Field | How to infer |
|-------|-------------|
| **Client** | From hint, git remote URL, CLAUDE.md, README, or ask |
...
Present each section to the user separately via **plain text** and wait for their response before moving to the next section. Do NOT show the full report at once.

### Confirmation 1 — Project Description
Display the drafted Section 1 fields, then ask:
> "¿Es correcta la descripción del proyecto? Responde 'sí' para continuar o describe los cambios necesarios."

Apply any requested corrections, then re-confirm before proceeding.

### Confirmation 2 — Mission Participation
Display the drafted Section 2 fields, then ask:
> "¿Son correctas las responsabilidades y actividades? Responde 'sí' para continuar o describe los cambios."

Apply any requested corrections, then re-confirm before proceeding.

### Confirmation 3 — Technical Competencies
Display the inferred competency list, then ask:
> "¿Son correctas las competencias técnicas? Deben pertenecer al catálogo <hr-platform>. Responde 'sí' para generar el informe o indica los cambios."

Apply any requested corrections, then re-confirm before finalizing.
