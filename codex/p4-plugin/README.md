...
Plugin lifecycle management for Codex CLI. Create, update, remove, and audit plugins and skills in the plugin4ai marketplace.
...
```bash
codex /plugins
# → Install plugin → enter path to codex/p4-plugin
```

## Notes

- This port manages plugins under `codex/<plugin>/` in the repo
- Plugin files are stored at `~/.codex/plugins/<plugin>/`
