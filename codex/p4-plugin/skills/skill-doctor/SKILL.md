...
skills_dir = f"{REPO_ROOT}/codex/{plugin_name}/skills"
```
...
For each skill directory found under `codex/<plugin>/skills/`:
...
No other fields are required for Codex CLI. Fields like `version`, `allowed-tools`, `argument-hint` are NOT expected and should not be flagged as missing.
...
The version in `codex/<plugin>/plugin.json` should match `plugin_entry["version"]` in catalog.json.
