...
specs/catalog.json               ← PRIMARY — written first
   ↓
codex/<plugin>/plugin.json       ← version if bumped
```
...
- `codex/<plugin>/plugin.json` → update `version` if bumped
...
Next steps:
  ...
  2. codex /plugins → update <plugin>   (reinstall to apply)
```
