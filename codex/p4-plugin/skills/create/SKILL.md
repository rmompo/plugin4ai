---
name: create
description: Scaffolds a new Codex CLI plugin with an initial skill, spec file, and catalog entry. Also invoked explicitly as /p4-plugin:create with the plugin name.
---

# Plugin Creator (Codex CLI)

Scaffolds a complete new plugin in the `codex/` directory and registers it in the catalog.
...
\`\`\`bash
codex /plugins
# → Install plugin → enter path to codex/<plugin>
\`\`\`
```
...
```json
{
  ...
  "ports": {
    ...
    "codex": { "status": "beta", "path": "codex/<plugin>" }
  },
  "dependencies": [],
  "changelog": [
...
Next steps:
  ...
  2. codex /plugins → Install plugin
