...
# Plugin Updater (Antigravity CLI)
...
specs/catalog.json                      ← PRIMARY — written first
   ↓
antigravity/<plugin>/plugin.json        ← version if bumped
```
...
- `antigravity/<plugin>/plugin.json` → update `version` if bumped
...
Next steps:
  ...
  2. agy plugin install ~/path/to/plugin4ai/antigravity/<plugin>   (reinstall to apply)
```
