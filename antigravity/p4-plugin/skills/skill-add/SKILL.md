---
name: skill-add
description: Adds a new skill to an existing Antigravity CLI plugin. Creates the SKILL.md, increments the Z version counter in catalog.json, and updates plugin.json and README.md. Also invoked explicitly as /p4-plugin:skill-add with plugin and skill name.
---

# Skill Adder (Antigravity CLI)
...
```python
# Global Z: max across all plugins in catalog
global_max_z = max(
...
```python
plugin_json_path = f"{REPO_ROOT}/antigravity/{plugin_name}/plugin.json"
...
Add a row to the skills table in `antigravity/<plugin>/README.md`:
