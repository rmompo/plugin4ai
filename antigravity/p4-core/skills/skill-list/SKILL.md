---
name: skill-list
description: Lists all p4-* plugin skills installed in the current CLI, in a table with columns plugin|skill|v|ready|description.
---
...
```bash
# Antigravity (agy)
CACHE_BASE=~/.gemini/config/plugins/cache/plugin4ai-antigravity
CATALOG=~/.p4/catalog.json
[ ! -f "$CATALOG" ] && CATALOG=~/.gemini/config/plugins/marketplaces/plugin4ai-antigravity/specs/catalog.json
```
