---
name: git-commit
description: This skill should be used whenever the agent is about to propose, write, or execute a git commit. It activates automatically when the user asks to commit changes, when running `git commit`, when generating commit messages after staged changes, or when the user asks for help writing a commit message. Also invoked explicitly as /p4-core:git-commit with an optional hint. Enforces Conventional Commits format, translates to English, and strips co-authorship lines.
---
...
2. **If invoked automatically** (agent is about to commit): inspect staged changes with `git diff --staged`, infer type and scope, generate the message
