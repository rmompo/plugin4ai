...
description: Loads and activates P4D behavioral directives for the session. Fires automatically at session start. Also invoked explicitly as /p4-core:model-behaviour to inspect or reload directives.
---

# P4D — Behavioral Directives (Antigravity CLI port)
...
Read this file from the plugin directory before doing anything else:

```bash
find ~/.gemini/config/plugins -path "*/p4-core/skills/model-behaviour/SKILL.md" 2>/dev/null | head -1
```
