...
allowed-tools: [Bash, Write, Edit, AskUserQuestion]
---
...
* text=auto eol=lf
```
(leading whitespace and inline comments are ignored when checking).
...
- question: `"N tracked file(s) have CRLF line endings. Normalize all to LF now?"`
- header: `"CRLF fix"`
- options:
  - `"Yes, normalize all"` — convert and stage
  - `"No, skip for now"` — exit without changes
...
When invoked as `/p4-core:git-sanitize`, always print a closing summary:
