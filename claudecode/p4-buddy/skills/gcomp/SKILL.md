...
version: 2
argument-hint: "[project description hint]"
...
Present each section to the user separately using **`AskUserQuestion`** before moving to the next. Do NOT ask via plain text — always use the `AskUserQuestion` tool for user input.

**Do NOT show the full report at once.** Follow this sequence:

### Confirmation 1 — Project Description
Display the drafted Section 1 fields, then call `AskUserQuestion` with:
- question: `"¿Es correcta la descripción del proyecto?"`
- header: `"Sección 1"`
- options: `["Sí, continuar", "Necesito hacer cambios"]`

If the user selects "Necesito hacer cambios": ask a follow-up `AskUserQuestion` with a free-text prompt, apply corrections, then re-confirm.

### Confirmation 2 — Mission Participation
Display the drafted Section 2 fields, then call `AskUserQuestion` with:
- question: `"¿Son correctas las responsabilidades y actividades?"`
- header: `"Sección 2"`
- options: `["Sí, continuar", "Necesito hacer cambios"]`

If the user selects "Necesito hacer cambios": ask a follow-up `AskUserQuestion`, apply corrections, then re-confirm.

### Confirmation 3 — Technical Competencies
Display the inferred competency list, then call `AskUserQuestion` with:
