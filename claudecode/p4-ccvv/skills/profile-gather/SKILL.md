---
name: profile-gather
description: Extracts structured profile data from a source CV file (PDF/DOCX) and saves it to the user's profile.json. Also invoked explicitly as /p4-ccvv:profile-gather.
version: 13
...
```bash
STORAGE_ROOT=~/.p4/p4-ccvv
PLUGIN_CACHE=$(ls -d ~/.claude/plugins/cache/plugin4ai-claudecode/p4-ccvv/*/ 2>/dev/null | sort -V | tail -1)
...
Use **AskUserQuestion** to select an existing profile or create a new one.
...
Use **AskUserQuestion** to ask the user for the **CV file path** (required): absolute or `~/`-prefixed path to their CV (`.pdf` or `.docx`).
...
Ask the user whether to delete the original. Only delete if confirmed — never delete silently:
```bash
rm "$CV_PATH"  # only if user confirms
```
...
- One option per image file found (show filename only, not full path)
- **Enter path manually** — user provides an absolute or `~/`-prefixed path to any image file
- **No photo** — skip photo entirely
...
```bash
cp "$PHOTO_PATH" "$STORAGE_ROOT/profiles/$PROFILE_ID/input/"
rm "$PHOTO_PATH"  # only if user confirms
```
Store the filename in `PHOTO_FILE`.
...
For missing or ambiguous fields use **AskUserQuestion** to clarify — never invent data.
...
Show the user which links were already found in the CV (pre-fill), then use **AskUserQuestion** to confirm or complete each tier:
...
|----------|----------|---------------|
| `medium` / `devto` / `hashnode` | Technical blog | Thought leadership, Senior/Lead/Architect roles |
...
- If a link was already found in the CV, show it and ask the user to confirm or correct it — do not add it again.
