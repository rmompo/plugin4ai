---
name: profile-update
description: Updates an existing profile.json with new or corrected data. Creates an automatic backup before any modification. Also invoked explicitly as /p4-ccvv:profile-update.
version: 12
...
Read `~/.p4/p4-ccvv/profiles.json`. Use **AskUserQuestion** to select the profile to update.
...
Present the current `contact.others` entries (if any) and allow the user to add, update or remove links. Supported platforms and their type keys:
...
| `credly` | Credly certifications |
| `npm` | NPM profile |
| `pypi` | PyPI profile |

For each entry: collect `type` (from the table above or a custom value) and `value` (valid URL). If the type already exists in `contact.others`, update its value — do not duplicate. Accept only valid URLs; if format looks wrong, ask again. Never store empty values.

Collect field values via **AskUserQuestion**. Never modify fields not explicitly requested.
