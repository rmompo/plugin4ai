...
```bash
rm -rf REPO_ROOT/ghcopilot/plugins/<plugin>/skills/<skill>/
```
...
Remove the row for `<skill>` from the skills table in `ghcopilot/plugins/<plugin>/README.md`.
...
Deleted:
  ghcopilot/plugins/<plugin>/skills/<skill>/
