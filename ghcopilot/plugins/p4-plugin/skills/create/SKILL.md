---
name: create
description: Scaffolds a new GitHub Copilot plugin with an initial skill, spec file, and catalog entry. Also invoked explicitly as /p4-plugin:create with the plugin name.
---

# Plugin Creator (GitHub Copilot)

Scaffolds a complete new plugin in the `ghcopilot/plugins/` directory and registers it in the catalog.

```
ghcopilot/plugins/<plugin>/
├── plugin.json
├── README.md
└── skills/
    └── <initial-skill>/
...
### `ghcopilot/plugins/<plugin>/README.md`
...
| CLI | Status | Path |
|-----|--------|------|
| GitHub Copilot | beta | `ghcopilot/plugins/<plugin>` |
...
```json
{
  ...
  "ports": {
    ...
    "ghcopilot": { "status": "beta", "path": "ghcopilot/plugins/<plugin>" },
    "antigravity": { "status": "not-applicable" },
...
Next steps:
  1. Edit ghcopilot/plugins/<plugin>/skills/<skill>/SKILL.md with skill instructions
