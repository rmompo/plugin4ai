---
name: any-to-md
description: Converts PDF, DOC, DOCX, PPT, and PPTX documents to structured Markdown using a 9-step pipeline with best-effort tool degradation. Also invoked explicitly as /p4-converter:any-to-md.
version: 4
...
```bash
PLUGIN_CACHE=$(ls -d ~/.ghcopilot/plugins/cache/plugin4ai-ghcopilot/p4-converter/*/ 2>/dev/null | sort -V | tail -1)
...
Use **ask_user** to collect the three conversion parameters (or resolve from environment / skill argument).
...
Use **ask_user** to ask whether to reprocess already-converted files.

Print inventory summary and Use **ask_user** to confirm the inventory before proceeding.:
```
══════════════════════════════════════════
  DOCUMENT INVENTORY
══════════════════════════════════════════
