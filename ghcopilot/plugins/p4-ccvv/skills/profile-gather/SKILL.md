...
Use **ask_user** to select an existing profile or create a new one.
...
Use **ask_user** to ask the user for the **CV file path** (required): absolute or `~/`-prefixed path to their CV (`.pdf` or `.docx`).
...
Use **ask_user** to present the options:
- One option per image file found (show filename only)
- **Enter path manually** — user provides an absolute or `~/`-prefixed path
...
For missing or ambiguous fields use **ask_user** to clarify — never invent data.
...
Show the user which links were already found in the CV (pre-fill), then use **ask_user** to confirm or complete each tier:
