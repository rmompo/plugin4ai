---
name: export
description: Exports a CV draft to a final format (PDF, DOCX, HTML, MD) using Pandoc with graceful degradation. Also invoked explicitly as /p4-ccvv:export.
version: 7
...
Read `cvs.json`. Use **ask_user** to select the CV metadata entry.
...
Options:
1. **Copiar al directorio actual** — copies the file to `$PWD`
